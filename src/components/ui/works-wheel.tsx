...
export function WorksWheel({
  ...
  return (
    <section
      aria-label={label}
      className={cn(
        "bg-[#05070b] text-[#f2f4f8] relative h-full min-h-[40rem] w-full overflow-hidden select-none",
        className,
      )}
      {...props}
    >
          ...
          {items.map((item, i) => {
            ...
            return (
              <React.Fragment key={item.title}>
                ...
                >
                  <span className="bg-[#12151f] shadow-[#000000]/40 relative block size-full overflow-hidden rounded-lg shadow-[0_18px_40px_-18px_var(--tw-shadow-color)] border border-white/10">
                    <img
                      src={item.image}
                      alt={item.title}
                      draggable={false}
                    ...
                    />
                    {action && item.href ? (
                      <span className="bg-black/80 text-white pointer-events-none absolute right-3 bottom-3 flex translate-y-1 items-center gap-1 rounded-full px-2.5 py-1 text-[0.7rem] opacity-0 backdrop-blur-sm transition group-hover:translate-y-0 group-hover:opacity-100 border border-white/20">
                        <svg
                          viewBox="0 0 12 12"
                          className="size-2.5"
                          aria-hidden="true"
      ...
      <div
        ref={labelRef}
        className="pointer-events-none absolute inset-0 grid place-items-center tracking-tight font-black uppercase text-white"
        style={{ fontSize: metrics.title, fontFamily: "'Arial Black', Impact, sans-serif" }}
      >
        {label}
      </div>
      <div
        ref={titleRef}
        className="pointer-events-none absolute top-1/2 left-[8%] -translate-y-1/2 tracking-tight opacity-0 font-black uppercase text-white"
        style={{ fontSize: metrics.title, fontFamily: "'Arial Black', Impact, sans-serif" }}
      >
        {items[active]?.title}
      </div>
      ...
      <ol
        className="text-neutral-500 absolute top-[7.5%] right-[2.5%] text-right leading-[1.75] font-mono tracking-widest hidden md:block max-h-[80vh] overflow-hidden"
        style={{ fontSize: metrics.index }}
      >
        {items.map((item, i) => (
          <li key={item.title} className="truncate max-w-[200px]">
            <button
              type="button"
              onClick={() => to(i + 1)}
              className={cn(
                "focus-visible:outline-white cursor-pointer transition-colors outline-none focus-visible:outline-1",
                i === active && "text-white font-bold",
              )}
            >
              {item.title}
            </button>
